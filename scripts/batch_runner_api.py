#!/usr/bin/env python3
"""
Batch Runner for ARAG using API-based embedding (no local model needed).
Supports concurrent execution and checkpoint resume.

Usage:
    python scripts/batch_runner_api.py \
...
from arag.tools.semantic_search_api import SemanticSearchToolAPI
from arag.tools.read_chunk import ReadChunkTool

logging.basicConfig(level=logging.ERROR)


class BatchRunnerAPI:
    """Batch runner using API-based embedding (no sentence-transformers needed)."""
    ...
    def __init__(
        ...
        # Pre-initialize shared tools (load embedding index only)
    ...
    def _init_shared_tools(self) -> ToolRegistry:
        """Initialize shared tools using API-based semantic search."""
        ...
        # Add API-based semantic search if index exists
        index_file = Path(index_dir) / "sentence_index.pkl"
        if index_file.exists():
            embedding_config = self.config.get('embedding', {})
            llm_config = self.config.get('llm', {})

            print(f"Using API-based semantic search with model: {embedding_config.get('model', 'text-embedding-v4')}")
            print(f"API base: {os.getenv('ARAG_BASE_URL', 'https://api.openai.com/v1')}")

            tools.register(SemanticSearchToolAPI(
                chunks_file=chunks_file,
                index_dir=index_dir,
                model_name=embedding_config.get('model', 'text-embedding-v4'),
                api_key=llm_config.get('api_key') or os.getenv('ARAG_API_KEY'),
                api_base=llm_config.get('base_url') or os.getenv('ARAG_BASE_URL')
            ))
            print("API-based semantic search initialized!")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="ARAG Batch Runner (API-based)")
